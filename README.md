...
- **Real sign-up or login** (the email/password form, or a magic-link invite) — creates/authenticates a real account against `backend/`, via `frontend/src/lib/apiClient.ts`. This exercises Milestones 2-4 end to end: registration, OTP verification, KYC, login, team & role management, team member profiles (notes, custom alert rules), Safeguarding Lead + archive status, searchable/scroll-capped team and invite lists with bulk resend/cancel/archive (Team), real RAG assignments with access codes, a per-member risk-and-support dashboard with per-RAG traffic-light status, the staged alert model + Actions pipeline, the Employee×RAG record (Overview/Conversations-stub/Alerts/Actions/content-free Audit Log), account settings, a live real login-history feed with search (Settings), the audit trail, and the onboarding video CMS — add/search/view/share a video and see live analytics. Everything *outside* those specific pages (dashboards, alert-rule governance, the Global Alert Library, the RAG page's own settings/knowledge tabs, Calendar) still renders from the mock store even for a real account, since the backend doesn't have that data yet.
