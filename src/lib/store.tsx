...
interface AppContextValue extends AppState {
  ...
  toggleAlertCategory: (ragId: string, categoryId: string) => void;
  addAlertCategory: (ragId: string, label: string, notifyEmails: string[]) => void;
  answerQuestion: (questionId: string, answer: string) => void;
  assignRagToUser: (ragId: string, userId: string) => string;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const createRag = useCallback((name: string, accessPassword: string) => {
    const newRag: Rag = {
      ...
      documents: [],
      alertCategories: [
        { id: id("ac"), label: "Question the RAG cannot answer", enabled: true, notifyEmails: ["morgan.ellis@brightcare.co.uk"] },
      ],
    };
  ...
  const toggleAlertCategory = useCallback((ragId: string, categoryId: string) => {
    setState((s) => ({
      ...s,
      rags: s.rags.map((r) =>
        r.id === ragId
          ? { ...r, alertCategories: r.alertCategories.map((c) => (c.id === categoryId ? { ...c, enabled: !c.enabled } : c)) }
          : r
      ),
    }));
  }, []);

  const addAlertCategory = useCallback((ragId: string, label: string, notifyEmails: string[]) => {
    setState((s) => ({
      ...s,
      rags: s.rags.map((r) =>
        r.id === ragId ? { ...r, alertCategories: [...r.alertCategories, { id: id("ac"), label, enabled: true, notifyEmails }] } : r
      ),
    }));
  }, []);
  ...
  const assignRagToUser = useCallback((ragId: string, userId: string) => {
    const user = seedUsers.find((u) => u.id === userId);
    const code = accessCode(user?.name ?? "US");
    setState((s) => ({
      ...s,
      ragAssignments: [...s.ragAssignments, { userId, ragId, accessCode: code, assignedAt: nowIso() }],
    }));
    return code;
  }, []);
